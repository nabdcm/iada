...
import { getAdminSecret, hashSecret } from "../_adminAuth";
...
export async function POST(req: Request) {
  try {
    ...
    const expectedUsername = (process.env.NABD_ADMIN_USERNAME ?? "").toLowerCase();
    const expectedPassword  =  process.env.NABD_ADMIN_PASSWORD ?? "";

    if (!expectedUsername || !expectedPassword) {
      return NextResponse.json({ error: "Admin credentials not configured (set NABD_ADMIN_USERNAME / NABD_ADMIN_PASSWORD)" }, { status: 503 });
    }
    ...
    const token  = Buffer.from(
      JSON.stringify({ auth: "1", expiry, sig: hashSecret(secret) })
    ).toString("base64");
