...
function ShareModal({ lang, clinicId, copied, setCopied, onClose }: {
  ...
  // توليد الرابط القصير عبر TinyURL عند فتح المودال
  useEffect(() => {
    if (!clinicId) return;
    ...
    fetch(`https://tinyurl.com/api-create.php?url=${encodeURIComponent(fullUrl)}`)
      ...
      .then(url => {
        if (url.startsWith("https://tinyurl.com/")) {
          setShortUrl(url.trim());
        } else {
          setShortError(true);
        }
      })
      .catch(() => setShortError(true))
      .finally(() => setShortLoading(false));
  }, [clinicId]);

  // الرابط الذي يُستخدم فعلياً: القصير إن توفّر، وإلا الكامل
  const activeUrl = shortUrl || fullUrl;
  ...
  const handleWhatsApp = () => {
    const msg = isAr
      ? `مرحباً، يمكنك حجز موعد في عيادتنا عبر الرابط:%0A${activeUrl}`
      : `Hello, book an appointment at our clinic:%0A${activeUrl}`;
    ...
    <div style={{ position:"fixed",inset:0,zIndex:200,display:"flex",alignItems:"center",justifyContent:"center" }}>
      ...
      <div style={{ position:"relative",zIndex:1,background:"#fff",borderRadius:20,width:"100%",maxWidth:440,boxShadow:"0 24px 80px rgba(8,99,186,.18)",animation:"modalIn .25s cubic-bezier(.4,0,.2,1)",overflow:"hidden" }}>
        <div style={{ background:"linear-gradient(135deg,#0863ba,#054a8c)",padding:"28px 28px 24px",textAlign:"center",position:"relative" }}>
          ...
          <p style={{ fontSize:13,color:"rgba(255,255,255,.75)" }}>{isAr?"شارك هذا الرابط القصير مع مرضاك":"Share this short link with your patients"}</p>
        </div>
        <div style={{ padding:"24px 28px" }}>
          <div style={{ marginBottom:20 }}>
            <div style={{ display:"flex",gap:8,alignItems:"center",background:"#f7f9fc",border:"1.5px solid #eef0f3",borderRadius:12,padding:"10px 14px",minHeight:50 }}>
              {shortLoading ? (
                <div style={{ flex:1,display:"flex",alignItems:"center",gap:8,color:"#aaa",fontSize:13 }}>
                  <div style={{ width:14,height:14,border:"2px solid #ddd",borderTopColor:"#0863ba",borderRadius:"50%",animation:"spin 1s linear infinite",flexShrink:0 }}/>
                  {isAr?"جاري إنشاء الرابط القصير...":"Generating short link..."}
                </div>
              ) : (
                <span style={{ flex:1,fontSize:14,color:"#0863ba",fontWeight:700,direction:"ltr",textAlign:"left",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>
                  {activeUrl}
                </span>
              )}
              <button
                ...
                style={{ flexShrink:0,padding:"7px 14px",background:copied?"#2e7d32":shortLoading?"#ccc":"#0863ba",color:"#fff",border:"none",borderRadius:8,fontSize:12,fontWeight:700,cursor:shortLoading?"not-allowed":"pointer",fontFamily:"Rubik,sans-serif",transition:"all .3s" }}>
                {copied?(isAr?"✓ تم":"✓ Copied!"):(isAr?"نسخ":"Copy")}
              </button>
            </div>
            {shortError && (
              <p style={{ fontSize:11,color:"#e67e22",marginTop:6,paddingInlineStart:4 }}>
                {isAr?"⚠ تعذّر اختصار الرابط، سيتم نسخ الرابط الكامل":"⚠ Could not shorten URL, full link will be used"}
              </p>
            )}
          </div>
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:10 }}>
            <button onClick={handleWhatsApp} disabled={shortLoading} style={{ display:"flex",alignItems:"center",justifyContent:"center",gap:8,padding:"12px",background:"rgba(37,211,102,.1)",color:"#128c7e",border:"1.5px solid rgba(37,211,102,.25)",borderRadius:12,fontFamily:"Rubik,sans-serif",fontSize:13,fontWeight:600,cursor:shortLoading?"not-allowed":"pointer",opacity:shortLoading?0.6:1 }}>
            ...
            <button onClick={handleCopy} disabled={shortLoading} style={{ display:"flex",alignItems:"center",justifyContent:"center",gap:8,padding:"12px",background:"rgba(8,99,186,.08)",color:"#0863ba",border:"1.5px solid rgba(8,99,186,.15)",borderRadius:12,fontFamily:"Rubik,sans-serif",fontSize:13,fontWeight:600,cursor:shortLoading?"not-allowed":"pointer",opacity:shortLoading?0.6:1 }}>
